...
public class DnaAnalysis {
	...
	public static int countSubsequences(String subSequence) {
		...
		while(i + subLength < DNA.length() + 1) {
			if(DNA.substring(i, i + subLength).equals(subSequence)) {
				count++;
			}
			i += 1;
		}
	...
	public static void main(String args[]) {
		...
		if(args.length != 1) {
			System.out.println("No command line arguements or incorrect number of command line arguements given.");
			return;
		} else {
			subSequence = args[0];
		}
